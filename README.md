...
- **效用惩罚**：在路由效用函数里，对"非当前会话模型"施加与 `stickiness`（默认 0.4）成正比的切模型惩罚，让路由器自己学会"不轻易换模型"。
- **硬约束首选**：若会话已绑定模型且其档位与目标档位漂移在允许范围内，直接作为首选（`reason=session_affinity`），跳过探索，并写回存储供下一轮复用。

适用边界：对**稳定多轮会话 / 高缓存命中率模型**，粘性几乎总是更优；对**一次性请求**或**复杂度剧烈跳变**的对话，亲和自动让位于真实复杂度。可用 `routing.session_affinity` 整体关闭或调参。

#### 1b.1 基准测试：缓存亲和 vs 朴素路由

`bench_cache_affinity.py` 真实驱动路由引擎（非 mock），对 5 段多轮对话（含 system prompt + 多轮 user/assistant，共 7,158 prompt tokens）分别跑了三种部署策略，按 DeepSeek 类定价（缓存命中省 75%~90%）估算输入成本：

| 策略 | 模型切换次数 | 前缀缓存 | 含义 |
|------|----------|---------|------|
| **无网关**（固定一个便宜高缓存模型，如直连 DeepSeek） | 0 | 热 | 典型用户直连 |
| **朴素路由**（每轮按复杂度重选、无会话记忆） | 8 | 冷（命中 0%） | 文章批评的做法 |
| **ChatRouter + 缓存亲和** | 1 | 热 | 本仓库实现的特性 |

外推到 1 万会话 / 月的输入成本：

| 缓存节省 | 无网关 $/mo | 朴素路由 $/mo | ChatRouter+亲和 $/mo | 亲和比朴素路由省 |
|---------|-----------|-------------|-------------------|--------------|
| 75% | 0.09 | 0.20 | 0.09 | **−53.6%** |
| 90% | 0.07 | 0.20 | 0.07 | **−64.3%** |

结论：**朴素每轮路由会切 8 次模型、打碎前缀缓存，使输入成本翻倍（0.09 → 0.20）；开启会话亲和后切换降到 1 次，缓存恢复热度，成本回到与"无网关固定模型"相同的下限，同时保留首轮复杂度感知选档能力。** 节省随会话量线性放大。
...
方法学前提：成本统一用 DeepSeek 类低价高缓存价格，避免档位价差干扰；假设多轮对话 80% 的 prompt 是可复用共享前缀（`SHARED_PREFIX_FRACTION=0.80`）；朴素路由因无会话记忆每次独立重选，前缀缓存冷（命中 0%）。
